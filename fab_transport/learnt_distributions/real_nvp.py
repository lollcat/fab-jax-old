...
from fab_transport.types import XPoints, LogProbs, HaikuDistribution
...
def make_realnvp_dist_funcs(
        ...
        @hk.transform
        def z_sample_and_log_prob(sample_shape: Tuple[int, ...] = ()) -> Tuple[XPoints, LogProbs]:
            """Returns z, p z under base, used to init AIS."""
            model = get_model()
            z, log_prob = model.distribution.sample_and_log_prob(seed=hk.next_rng_key(),
                                                                 sample_shape=sample_shape)
            return z, log_prob

        @hk.without_apply_rng
        @hk.transform
        def z_x_and_log_det_forward(z: chex.Array) -> Tuple[XPoints, chex.Array]:
            model = get_model()
        ...
        @hk.without_apply_rng
        @hk.transform
        def z_log_prob(z: chex.Array) -> Tuple[XPoints, chex.Array]:
            model = get_model()
            return model.distribution.log_prob(z)
        ...
        return HaikuDistribution(
            dim=x_ndim,
            log_prob=log_prob,
            sample_and_log_prob=sample_and_log_prob,
            sample=sample,
            z_sample_and_log_prob=z_sample_and_log_prob,
            z_x_and_log_det_forward=z_x_and_log_det_forward,
...
def make_flow_model(event_shape: Sequence[int],
                    ...
                    layer_norm: bool,
                    act_norm: bool,
                    tunable_base: bool = False) -> distrax.Transformed:
